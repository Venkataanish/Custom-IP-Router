...
void populate_node3(struct ifaddrs *iap, struct routing_table arp_entry[],
		int i) {
	char buf[32];
	...
	if (iap->ifa_addr && (iap->ifa_flags & IFF_UP)
		...
		if ((strcmp("10.1.2.1", buf)) == 0) {
			...
			unsigned char t_ip[4] = { 10, 1, 2, 3 }; //node 3
		...
		for (index = 0; index < 6; index++) {
			/*eth[i].source_mac[index] = (uint8_t) arp_req->sender_mac[index];*/
		...
		/*Setting protocol
		 of the
		 packet*/send_req->h_proto = htons(ETH_P_ARP);

		/*Creating ARP
		 request*/arp_req->hardware_type = htons(HW_TYPE);
		...
		while (1) {
			...
			if (htons(rcv_resp->h_proto) == PROTO_ARP) {
				arp_entry[i].destination_ip = (uint32_t) arp_resp->sender_ip[3]
						<< 24 | (uint32_t) arp_resp->sender_ip[2] << 16
...
void get_packet(struct routing_table arp_entry[], EthMacPair eth[]) {
	int i;
	struct ifaddrs *addrs, *iap, *node3_if;
	struct sockaddr_in *sa;
	char buf[32];
	unsigned char source_ip[4];
	unsigned char target_ip[4];
	...
	i++;
	populate_node3(node3_if, arp_entry, i);
}
