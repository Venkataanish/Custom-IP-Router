...
void get_packet(struct arp_table arp_entry[2], EthMacPair eth[2])
{
    ...
    for (i = 0, iap = addrs; iap != NULL; iap = iap->ifa_next) {
        if (iap->ifa_addr && (iap->ifa_flags & IFF_UP) && iap->ifa_addr->sa_family == AF_INET){
            ...
            if ((strcmp("10.1.2.1", buf) == 0) || (strcmp("10.10.1.2", buf) == 0)){
            ...
            struct arp_header *arp_req = (struct arp_header *) (buffer + ETH_HEADER_LEN);
            struct arp_header *arp_resp = (struct arp_header *) (buffer + ETH_HEADER_LEN);
            ...
            for (index = 0; index < 6; index++){
                eth[i].sender_mac[index] = (uint8_t) arp_req->sender_mac[index];
                arp_entry[i].sender_mac[index] = (uint8_t) arp_req->sender_mac[index];
            }
            ...
            while (1) {
                ...
                if (htons(rcv_resp->h_proto) == PROTO_ARP) {
                    ...
                    for (index = 0; index < 6; index++) {
                        arp_entry[i].destination_mac[index] = (uint8_t) arp_resp->sender_mac[index];
                    }
                    i++; 
                    break;
                } //if
